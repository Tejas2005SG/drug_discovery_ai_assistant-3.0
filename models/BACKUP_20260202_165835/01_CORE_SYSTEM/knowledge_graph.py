...
class DrugRepurposingKnowledgeGraph:
    ...
    def find_diseases_by_symptoms(self, symptom_names, top_k=5):
        ...
        if not matched_symptoms:
            # UNIVERSAL FALLBACK: If no exact matches, try partial word matching
            print("    [KG] No exact symptom matches. Trying partial matching...")
            ...
            for sym_name in symptom_names:
                ...
                for sym_id, name in self.symptoms.items():
                    ...
                    common_words = sym_words & name_words
                    if common_words:
                        matched_symptoms.append(sym_id)
            
            # If still no matches, create dynamic entries
            if not matched_symptoms:
                print("    [KG] Creating dynamic symptom entries...")
                universal_disease_id = "universal_condition"
                
                # Create a universal disease if it doesn't exist
                if universal_disease_id not in self.diseases:
                    self.diseases[universal_disease_id] = {
                        'name': 'Universal Multi-System Condition',
                        'symptoms': [],
                        'drugs': [],  # Add empty drugs list
                        'description': 'Auto-generated condition for unknown symptom patterns'
                    }
                    print("    [KG] Created universal disease: Universal Multi-System Condition")
                
                # Ensure drugs key exists
                if 'drugs' not in self.diseases[universal_disease_id]:
                    self.diseases[universal_disease_id]['drugs'] = []
                
                # Clear old dynamic symptoms from the universal disease
                self.diseases[universal_disease_id]['symptoms'] = []
                
                for i, sym_name in enumerate(symptom_names):
                    new_sym_id = f"dynamic_symptom_{i}_{hash(sym_name) % 10000}"
                    if new_sym_id not in self.symptoms:
                        self.symptoms[new_sym_id] = sym_name
                        # Add to graph with random embedding
                        if self.entity_embeddings is not None:
                            if new_sym_id not in self.entity_embeddings:
                                # Get dimension from existing embeddings
                                dim = 100  # Default dimension
                                if len(self.entity_embeddings) > 0:
                                    dim = len(next(iter(self.entity_embeddings.values())))
                                self.entity_embeddings[new_sym_id] = np.random.randn(dim).astype(np.float32) * 0.01
                    
                    matched_symptoms.append(new_sym_id)
                    # Link symptom to universal disease
                    if new_sym_id not in self.diseases[universal_disease_id]['symptoms']:
                        self.diseases[universal_disease_id]['symptoms'].append(new_sym_id)
                
                print(f"    [KG] Linked {len(matched_symptoms)} dynamic symptoms to universal disease")
                
                # Link ALL available drugs to universal disease for fallback
                if len(self.diseases[universal_disease_id]['drugs']) == 0:
                    # Add up to 20 random drugs from database
                    import random
                    available_drugs = list(self.drugs.keys())
                    if len(available_drugs) > 20:
                        selected_drugs = random.sample(available_drugs, 20)
                    else:
                        selected_drugs = available_drugs
                    self.diseases[universal_disease_id]['drugs'] = selected_drugs
                    print(f"    [KG] Linked {len(selected_drugs)} fallback drugs to universal disease")
