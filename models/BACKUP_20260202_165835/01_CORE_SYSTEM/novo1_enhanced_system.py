...
class EnhancedDrugGenerator:
    ...
    def identify_target_proteins(self, symptoms: List[str], top_k: int = 5) -> Dict:
        ...
        if not matched_diseases:
            ...
            return {'target_proteins': [], 'matched_diseases': []}
    ...
    def generate_candidates(self, symptoms: List[str], n_candidates: int = 10) -> Dict:
        ...
        if not source_drugs:
            ...
            # Get any available drugs as fallback
            available_drugs = []
            for drug_id, drug_data in self.kg.drugs.items():
                if 'smiles' in drug_data and len(drug_data['smiles']) > 5 and 'PROTEIN' not in drug_data['smiles']:
                    available_drugs.append({
                        'drug_id': drug_id,
                        'name': drug_data['name'],
                        'smiles': drug_data['smiles'],
                        'targets': drug_data.get('targets', ['Unknown']),
                        'score': 0.5  # Default score
                    })
            
            # Take up to 10 random drugs
            if available_drugs:
                import random
                source_drugs = random.sample(available_drugs, min(5, len(available_drugs)))
                print(f"[OK] Using {len(source_drugs)} fallback drugs from database")
            else:
                print("[WARNING] No valid drugs available in database")
                return {**target_data, 'candidates': []}
